              ...
              if (err.errors) {
                ...
                if (err.errors.title) {
                  ...
                  if (err.errors.body) {
                    ...
                    console.log(err); // Return general error message
                  }
                  if (err.errors.cas) {
                    res.json({ success: false, message: err.errors.body.message }); // Return error message
                  } else {
              ...
              if (err) {
                ...
                if (!user) {
                  ...
                  if (user.username !== blog.createdBy) {
                    ...
                    if (req.body.cas !=="ontheway" && req.body.cas !=="delivered" && req.body.cas !=="processing") {
                      res.json({ success: false, message: 'Status must be only ontheway ,delivered or processing' });
                    }else{
                    blog.title = req.body.title; // Save latest blog title
                    blog.body = req.body.body; // Save latest body
                    blog.cas = req.body.cas; // Save latest body
                    blog.save((err) => {
                      if (err) {
                        if (err.errors) {
